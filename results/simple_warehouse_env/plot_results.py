...
def plot_models(model_names, features):
    n_agents = features[0]
    features = features[1:]
    ...
    for i, feature in enumerate(features):
        for j, model_name in enumerate(model_names):
            file_path = os.path.join(model_name, f"{model_name}.csv")
            print(file_path)
        ...
        ax[coords[i]].set_xticks(df[n_agents])
        ax[coords[i]].set_title(f"{feature} Comparison")
        ax[coords[i]].legend()
    plt.show()

if __name__ == "__main__":
    models = ['DCC', 'PRIMAL_256', 'SCRIMP', 'AB-MAPPER']
    features = ['n_agents', 'success_rate', 'episode_length', 'total_step', 'collision_rate', 'extra_time', 'avg_step']
    plot_models(models, features)